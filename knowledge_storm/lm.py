...
class VLLMClient(dspy.dsp.LM):
    ...
    def __call__(self, prompt: str, **kwargs):
        ...
        if choices is not None:
            ...
            print("Warning: No choices returned from the API.")
            completions = []  # Handle the absence of choices appropriately
...
class OllamaClient(dspy.OllamaLocal):
    ...
    def __init__(self, model, port, url=ollamaHost, **kwargs):
        ...
        if not url.startswith("http://") and not url.startswith("https://"):
            url = "http://" + url
        super().__init__(model=model, base_url=f"{url}:{port}", timeout_s=1200, **kwargs)
        # Store additional kwargs for the generate method.
        self.kwargs = {**self.kwargs, **kwargs}
