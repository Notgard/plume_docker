...
class WikiWriter(dspy.Module):
    ...
    def __init__(self, engine: Union[dspy.dsp.LM, dspy.dsp.HFModel]):
        ...
        # CoT might bug with Ollama, see extract_question
        self.ask_question = dspy.ChainOfThought(AskQuestion)
        self.engine = engine
    
    def extract_question(self, response):
        # response may start with the inputs of the signature with Ollama. In this case capture "Question:". Sometimes there is nothing after "Question:"
        match = re.search(r'\bquestion\b\s*[:-]+\s*(.*?\?)', response, re.IGNORECASE)
        if match:
            return match.group(1).strip()
        else:
            if response.startswith("---\n\nTopic"):
                response = ""
                # the generation of the question failed
            return response
    ...
    def forward(
        ...
        with dspy.settings.context(lm=self.engine):
            ...
            question=self.extract_question(question)
            ...
            if len(searched_results) > 0:
                ...
                for r in searched_results:
                    if r.url not in used_urls :
                        ...
                        selected_snippets = []
                        encoded_query = model.encode(question, show_progress_bar=False)
                        encoded_snippets = model.encode(r.snippets, show_progress_bar=False)
                        sim = cosine_similarity([encoded_query], encoded_snippets)[0]
                        # we could also make the similarity researched ranking on all the snippets together to limit the size of info but it wouldn't assure that every source is used
                        sorted_indices = np.argsort(sim)
                        
                        for i in sorted_indices[-keep_top_k:][::-1]:
                            if sim[i]<0.2 :
                                break
                            selected_snippets.append(r.snippets[i])
...
class StormKnowledgeCurationModule(KnowledgeCurationModule):
    ...
    def _get_considered_personas(self, topic: str, database:DataBase, max_num_persona) -> List[str]:
        return self.persona_generator.generate_persona(
            topic=topic, database=database, max_num_persona=max_num_persona
        )
    ...
    def _run_conversation(
        ...
        def run_conv(persona, topic: str, database: DataBase):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            future_to_persona = {
                executor.submit(run_conv, persona, topic, database): persona
    ...
    def research(
        ...
        if disable_perspective:
            ...
            considered_personas = self._get_considered_personas(
                topic=topic, database=database, max_num_persona=max_perspective
            )
