...
from knowledge_storm.database import Topic, JSONDocument, MDDocument
from sqlalchemy import desc

from ...interface import Information, InformationTable, Article, ArticleSectionNode
from ...utils import ArticleTextProcessing, FileIOHelper, DemoTextProcessingHelper
...
class StormInformationTable(InformationTable):
    ...
    def dump_url_to_info(self, path, client, database, topic):
        url_to_info = copy.deepcopy(self.url_to_info)
        for url in url_to_info:
            url_to_info[url] = url_to_info[url].to_dict()
        json_data = json.dumps(url_to_info)
        json_bytes = BytesIO(json_data.encode("utf-8"))

        try:
            client.put_object(minioBucket, path, data=json_bytes, length = len(json_data), content_type = "application/json")
        except S3Error as e:
            print(f"Error while creating 'raw_search_results.json' : {e}")
    ...
    @classmethod
    def from_conversation_log_file(cls, path, client):
        try:
            response = client.get_object(minioBucket, path)
        except S3Error as e:
            print(f"Error loading the log file : {e}")
        
        conversation_log_data = json.load(response)
        response.close()
...
class StormArticle(Article):
    ...
    def insert_or_create_section(
        ...
        parent_node: Optional[ArticleSectionNode] = None,
        trim_children=False,
    ):
        if parent_node==None :
        ...
        for section_name, content_dict in article_dict.items():
            ...
            if current_section_node is None:
                ...
                insert_to_front = (
                    ...
                    and (current_section_node.section_name == "Summary" or current_section_node.section_name == "Résumé")
                )
            ...
            self.insert_or_create_section(
                ...
                parent_node=current_section_node,
                trim_children=True,
            )
    ...
    def update_section(
        ...
        if parent_node is None:
            parent_node=self.root
        print(f"Current section content: {current_section_content}")
        article_dict = ArticleTextProcessing.parse_article_into_dict(
            current_section_content
        )
        print(f"Article Dictionnary: {article_dict}")
        
        self.update_first_level_section_name(section_name, article_dict)
    ...
    def reorder_reference_index(self):
        # Pre-order traversal to get order of references in the article
        ref_indices = []
        print ([child.section_name for child in self.root.children])
        ...
        # Update content with the correct reference index
    ...
    @classmethod
    def from_outline_file(cls, topic: str, file_path: str, client):
        """
        Create StormArticle class instance from outline file.
        """
        outline_str_response = client.get_object(minioBucket, file_path)
        outline_str = outline_str_response.read().decode('utf-8')
        outline_str_response.close()
        outline_str_response.release_conn()
    ...
    def from_outline_str(cls, topic: str, outline_str: str):
        ...
        if lines:
            ...
            for line in lines:
                if adjust_level and line.startswith("#") and not line.startswith("##"):
                    adjust_level = False
    ...
    def dump_outline_to_file(self, file_path, client, database, topic):
        outline = self.get_outline_as_list(add_hashtags=True, include_root=False)
        outline_str = "\n".join(outline)

        markdown = BytesIO(outline_str.encode("utf-8"))

        try:
            client.put_object(minioBucket, file_path, data=markdown, length = len(outline_str), content_type = "application/json")
        except S3Error as e:
    ...
    def dump_article_as_plain_text(self, file_path, client, database, topic):
        text = self.to_string()
    ...
    def add_reference(self):
        content = DemoTextProcessingHelper.construct_bibliography_from_url_to_info(self.reference)
        self.root.children.append(ArticleSectionNode("Références", content))
        
    def link_citation(self):
        
        def inline_citation_to_link(node):
            if node is not None:
                if node.content is not None and node.content:
                    node.content = DemoTextProcessingHelper.add_inline_citation_link(
                        node.content,
                        self.reference["url_to_unified_index"]
                        )
                for child in node.children:
                    inline_citation_to_link(child)

        inline_citation_to_link(self.root)
        
    def update_first_level_section_name(self, section_name : str, article_dict : Dict[str, Dict]) :
        # could be more robust to sections generated with two #titles
        section_node=self.find_section(self.root, section_name)
        if article_dict:
            new_name = next(iter(article_dict))
        else:
            new_name = None
        section_node.section_name=new_name