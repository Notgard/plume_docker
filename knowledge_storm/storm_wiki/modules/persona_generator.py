...
class CreateWriterWithPersona(dspy.Module):
    ...
    def forward(self, topic: str, draft=None):
        with dspy.settings.context(lm=self.engine):
            ...
            # Ollama is having trouble with chain of thoughs with to much context
            # you can switch to dspy.Predict in this case
        personas = []
...
class StormPersonaGenerator:
    ...
    def generate_persona(self, topic: str, database: DataBase, max_num_persona: int = 3) -> List[str]:
        """
        ...
        """
        database.update_status_topic(topic=topic, status='Génération des "personas"...')
        ...
        database.update_status_topic(topic=topic, status=status)
